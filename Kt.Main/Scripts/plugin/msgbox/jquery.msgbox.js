 ...
 * 
 * Author: pwwang Website: pwwang.com Note: All the stuff written by pwwang Feel
 * free to do whatever you want with this file Please keep the distribution
 * 
 */
(function($) {
    ...
    $.msgbox = function(o) {
        ...
        if (typeof (o) == 'string') { o = { content: o} }                          // 如果参数给出字符串，
																					// 则直接进行提示（text）
        var opts = o || {};                                                     // 用于接收参数

        opts.width = o.width || 360;                    // 提示框的宽度
        opts.height = o.height || 200,                    // 提示框的高度
		opts.autoClose = o.autoClose || 0;                      // 自动关闭的时间,
																// 0则不会自动关闭
        ...
        opts.inputboxClass = o.inputboxClass || 'msgbox_inputbox';      // 内容框中input
																		// box的class
        opts.type = o.type || 'text';                 // 内容的类型
        // support: text, url(=get), iframe, confirm, alert; confirm, alert is
		// added in version 4.0, input added in V5.0
        opts.content = o.content || 'Hello, world!';        // 内容
        opts.onClose = o.onClose || function() { };          // 关闭时执行的事件
        opts.closeImg = o.closeImg || '';                     // 关闭按钮图标,
																// 空则显示"关闭"
        opts.bgOpacity = o.bgOpacity || 0.6;   			            // from 0 to
																	// 1 背景透明度
        ...
        function autoCloseMe(autoClose) {
            if (autoClose > 0 && isVisible()) { // prevent manually closing,
												// 防止人为关闭后,计时器还在运行
        ...
        function fixBox() {  // 定位box
            $wrapper.css({
            'top': $(document.documentElement).scrollTop() + relTop + 'px',
        ...
        $(window)
			.load(resetPosition)		// just in case user is changing size of
										// page while loading
        ...
        if (opts.enableDrag)
            $wrapper.Drags({  // 允许拖拽
                ...
                onDrop: function() {
                    relTop = $wrapper.getCss('top') - $(document.documentElement).scrollTop();
    ...
    $.closemsgbox = function(o) {
        ...
        if (opts.boxAnimate)
            $wrapper.slideUp('slow',function(){$wrapper.remove();});
        else
        	$wrapper.remove();
        if (opts.bgAnimate)
